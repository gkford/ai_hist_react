...
export function RTSliderCard({
  ...
  const resourcesForDisplay = transformation?.inbound
    .concat(transformation?.outbound || [])
  ...
  const handlePriorityClick = () => {
    ...
    const otherActiveRTs = Object.entries(states).filter(
      ([id, state]) => id !== rtId && state.priority !== 'none'
    )
  ...
  const simplifiedText = `⚡ → ${transformation.outbound
    .map((item) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className={cn(
                ...
                rtState.priority === 'high'
                  ? 'bg-green-500 hover:bg-green-600 text-white'
                  : rtState.priority === 'low'
