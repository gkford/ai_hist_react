...
export function ThoughtApplicationDialog() {
  ...
  // Find all unlocked cards
  const unlockedCards = Object.values(cardStates)
    .filter((card) => card.discovery_state.current_status === 'unlocked')
    .sort(
      (a, b) =>
        a.discovery_state.thought_level - b.discovery_state.thought_level
    )
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full">
        <h2 className="text-xl font-bold mb-4">
          Where would you like to apply your thoughts?
        </h2>

        {unlockedCards.length === 0 ? (
          <p className="text-gray-600 mb-4">
            No discoveries available to apply thoughts to.
          </p>
        ) : (
          <div className="space-y-3 max-h-[60vh] overflow-y-auto mb-4">
            {unlockedCards.map((card) => (
              ...
              >
                <div>
                  <div className="italic text-gray-700">{card.title}</div>
                  <div className="text-sm text-gray-500">
                    Requires{' '}
                    {
                      WORKER_ICONS[
                        card.discovery_state
                          .thought_level as keyof typeof WORKER_ICONS
                      ]
                    }{' '}
                    level thoughts
                  </div>
