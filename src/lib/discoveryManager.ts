...
export function processDiscoveries() {
  console.log("=== Discovery Processing Start ===");
  const resourceStore = useResourceStore.getState();
  const cardStore = useCardsStore.getState();
  ...
  console.log("Thoughts produced this turn:", thoughtsProduced);
  
  if (thoughtsProduced <= 0) {
    console.log("No thoughts produced, skipping discovery processing");
    return;
  }
  ...
  console.log("Eligible cards:", eligibleCards.map(([id]) => id));
  ...
  if (totalFocusPoints <= 0) {
    console.log("No focus points allocated, skipping discovery processing");
    return;
  }
  ...
  eligibleCards.forEach(([cardId, card]) => {
    ...
    console.log(`Focus priority: ${cardFocus}, proportion: ${thoughtsProportion}`);

    // Calculate thoughts to invest in this card
    const thoughtsToInvest = thoughtsProduced * thoughtsProportion;
    console.log(`Thoughts to invest: ${thoughtsToInvest}`);
    ...
    if (thoughtsToInvest > 0) {
      ...
      console.log(`New total thoughts invested: ${newThoughtInvested}`);
      console.log(`Current thresholds - To imagine: ${card.discovery_state.thought_to_imagine}, To discover: ${card.discovery_state.further_thought_to_discover}`);
  ...
  console.log("=== Discovery Processing End ===");
}
